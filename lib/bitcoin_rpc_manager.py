...
class BitcoinRPCManager(object):
    ...
    def check_height(self):
        while True:
            try:
                resp = (yield self.conns[self.curr_conn]._call('getblocktemplate', []))
                break
            except:
                log.error("Check Height -- Pool %i Down!" % (self.curr_conn) )
                self.next_connection()
        curr_height = json.loads(resp)['result']['height']
        ...
        for i in self.conns:
            ...
            try:
                resp = (yield self.conns[i]._call('getblocktemplate', []))
            except:
                log.error("Check Height -- Pool %i Down!" % (i,) )
                continue

            height = json.loads(resp)['result']['height']
