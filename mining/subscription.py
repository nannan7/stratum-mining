...
class MiningSubscription(Subscription):
            ...
            try:
                if subscription != None:
                    ...
                    if session['authorized'].keys():
                        ...
                        subscription.emit_single(work_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, hashstateroot, hashutxoroot, clean_jobs)
                    else:
                        subscription.emit_single(job_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, hashstateroot, hashutxoroot, clean_jobs)
    ...
    def _finish_after_subscribe(self, result):
        ...
        clean_jobs = True
        self.emit_single(job_id, prevhash, coinb1, coinb2, merkle_branch, version, nbits, ntime, hashstateroot, hashutxoroot, True)
